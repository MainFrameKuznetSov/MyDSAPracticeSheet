...
class TrappingRainWater
{
	public static void main(String[] args) 
	{
		...
		{
		    int n=ms.nextInt();
		    int arr[]=ms.readArray(n);
		    System.out.println(getRainWaterLinear(arr));
		}
		pw.close();
	}
	static int getRainWaterLinear(int x[])
    {
        int len=x.length;
        ...
        for(int i=1;i<len-1;i++)
            leftMax[i]=Math.max(x[i],leftMax[i-1]);
        rightMax[len-1]=x[len-1];
        for(int i=len-2;i>=0;i--)
            rightMax[i]=Math.max(x[i],rightMax[i+1]);
        int water=0;
        for(int i=1;i<len-1;i++)
            water+=Math.min(leftMax[i],rightMax[i])-x[i];
        return water;
    }
