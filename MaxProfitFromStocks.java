...
class BuyAndSellStocks
{
	public static void main(String[] args) 
	{
		...
		while(t-->0)
		{
		    ...
		    pw.println(getMaxProfit(arr));
		}
		pw.close();
	}
	static int getMaxProfit(int x[])
	{
	    int len=x.length,maxProfit=0;
	    int profit[]=new int[len];
	    profit[len-1]=x[len-1];
	    for(int i=len-2;i>=0;i--)
	        profit[i]=Math.max(x[i],profit[i+1]);
	    for(int i=0;i<len;i++)
	        maxProfit=Math.max(maxProfit,profit[i]-x[i]);
	    //printArray(profit);
	    return maxProfit;
	}
